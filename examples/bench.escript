...
main([HostPort, Pubs, Subs, StrPublishCount, Subject, Payload]) ->
    ...
    prepare_bench(BinHost, Port, NPubs, NSubs, PublishCount, BinSubject, BinPayload),
...
prepare_bench(Host, Port, NPubs, NSubs, PublishCount, Subject, Payload) ->
    ...
    {Time, ok} = timer:tc(fun() -> bench(Pubs, NSubs) end),
...
loop(NSubs, CompletedSubs) when NSubs == CompletedSubs ->
    ok;

loop(NSubs, CompletedSubs) ->
    receive
        {done, _MC} -> loop(NSubs, CompletedSubs + 1);
        _Other -> ok
    end.
...
subscriber_loop(Parent, _Conn, _Subject, PublishCount, MsgCount)
        when PublishCount == MsgCount ->
...
wait_for_subscribers(NSubs, ReadySubs) when NSubs == ReadySubs ->
    ok;
...
wait_for_publishers(NPubs, ReadyPubs) when NPubs == ReadyPubs ->
    ok;
